...
public class NetworkLoggerPlugin<Target: MoyaTarget>: Plugin<Target> {
    ...
    public override func didReceiveResponse(response: MoyaResponse?, error: MoyaError?, provider: MoyaProvider<Target>, target: Target) {
        logNetworkResponse(response, target: target)
    }
...
private extension NetworkLoggerPlugin {
    ...
    func logNetworkResponse(response: MoyaResponse?, target: Target) {
        ...
        if let stringData = NSString(data: response.data, encoding: NSUTF8StringEncoding) as? String where verbose == true {
            output += stringData
        }
